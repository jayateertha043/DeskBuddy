    ...
    namespace
    {
        ...
        void handleRoot()
        {
            ...
            page += F("</div><button class=save type=submit>Save behavior</button>"
                      "</form></div>");
            page += F("<div class=card><h2>Focus timer</h2>"
                      "<p class=hint>Pomodoro with an animated hourglass. Buddy celebrates when time is up.</p>");
            if (Pomodoro::running())
            {
                const uint32_t totalSec = (Pomodoro::remainingMs() + 999) / 1000;
                page += F("<div class=st>Running \xC2\xB7 ");
                page += String(totalSec / 60);
                page += ':';
                if (totalSec % 60 < 10)
                    page += '0';
                page += String(totalSec % 60);
            ...
            page += F("<form method=POST action=/pomodoro>"
                      "<label>Session length \xC2\xB7 <b id=mv>");
            page += String(Settings::pomodoroMinutes());
            page += F("</b> min</label>"
                      "<input type=range name=mins min=1 max=60 step=1 value=");
            page += String(Settings::pomodoroMinutes());
            page += F(" oninput=\"document.getElementById('mv').textContent=this.value\">"
                      "<div class=mode>"
